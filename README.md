...
| 項目                      | 說明                                                                                                                                |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| ✨ 8% 重量安全係數        | `calculateQuote()` 在查運費費率表**前**先將總重量 × 1.08，避免單品估算誤差導致運費不足。`/price` 試算頁是手動輸入，**不套用此係數** |
| 🐛 移除多餘 1% 運費附加費 | 原 `getShippingTwd()` 有 `costJpy × 1.01`，已改為由上層重量緩衝取代，避免雙重計算                                                   |
| ✨ Uniqlo 涼鞋重量修正    | Uniqlo API `category.locale` 含「サンダル」→ 自動歸類為 `accessories\|sandals`（650g），與皮鞋/靴子（1200g）分開估算                |
