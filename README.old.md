...
LINE 官方帳號的日本代購服務，整合 **Uniqlo**、**GU**、**RS Taichi（重機）**、**Kushitani（重機）**、**FR2（潮流服飾）**、**BAPE（潮流服飾）** 等多品牌商品查詢、購物車、結帳、訂單管理、Google 試算表同步及自動化運維。
...
- `useExchangeRate.ts`（前端）、`exchangeRate.ts`（後端爨蟲） 同步套用
...
- `webhook.post.ts` 在爬蟲前檢查禁售清單，命中時回覆營用提示
...
| 開始購物品牌分類          | `server/utils/line/shopCarousel.ts` → `buildCategorySelector()`、`fashionBrands`、`motogearBrands`                                                               |
