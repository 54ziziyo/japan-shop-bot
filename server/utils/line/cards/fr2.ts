// server/utils/line/cards/fr2.ts
...
export function buildFr2Cards(
  ...
  return productData.variants.map((v: any) => {
    ...
    // 壓縮圖片 URL（LINE postback 上限 300 字元）
    const imgCompact = (v.image || '').replace(
      /^https?:\/\/cdn\.shopify\.com\/s\/files\/1\/0288\/8515\/5937\//,
      'FR2:',
    );

    const sizeButtons: FlexComponent[] = v.sizes.map((s: any) => {
      const baseData = `action=buy&brand=fr2&c=${encodeURIComponent(v.color)}&s=${encodeURIComponent(s.name)}&p=${encodeURIComponent(v.price)}&code=${productData.handle}&img=${encodeURIComponent(imgCompact)}&cat=fr2|${productData.weightGrams}&ts=${Math.floor(Date.now() / 1000)}`;
