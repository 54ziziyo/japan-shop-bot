...
export default defineEventHandler(async (event) => {
  ...
  if (couponCode) {
    const { data: coupon } = await supabase
      .from('coupon_codes')
      .select(
        'id, code, discount_twd, discount_rules, used_count, total_quantity, is_active, expires_at, per_user_limit',
      )
      .eq('code', normalizeCouponCode(couponCode))
      .maybeSingle();
  ...
  const couponSummaryText =
    couponDiscountTwd > 0 && couponCode
      ? `折扣碼優惠${couponCode ? `（${couponCode}）` : ''}：-NT$${couponDiscountTwd.toLocaleString()}`
      : '';
  ...
  const adminMsg = [
    ...
    `  ─────────`,
    couponSummaryText ? `  ${couponSummaryText}` : '',
  ...
  <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#333">
    ...
    <div style="background:#fff;border:1px solid #e8e8e8;padding:20px 24px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        ...
        ${couponDiscountTwd > 0 ? `<tr><td style="padding:3px 0;color:#c0392b">折扣碼優惠${couponCode ? `（${couponCode}）` : ''}</td><td style="text-align:right;color:#c0392b">-NT$${couponDiscountTwd.toLocaleString()}</td></tr>` : ''}
